...
function getAddendaTypeCode(type) {
  let typeCode
  switch (Number(type)) {
    case 2:
      typeCode = 2;
  ...
  return typeCode.toString().padStart(2, '0');
}
...
createEntry = function(whichKind, entryData) {
  ...
  entry = {
    ...
    addendaIndicator: typeof entryData.addenda?.info  === 'string' ? '1' : '0',
  ...
  if (typeof entryData.addenda === 'object' && typeof entryData.addenda.info === 'string') {
    let { type, info } = entryData.addenda
    entry.addenda = {
      recordType: '7',
      type: getAddendaTypeCode(type),
      num: 1, // CCD, PPD, and WEB only allow 1 addenda record
      // Entry detail sequence number is last 7 digits of entry's trace number
      entryDetail: entry.traceNumber % 10_000_000,
      info: info
    };
  }
