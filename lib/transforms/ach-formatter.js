const batch = require('../api/create/batch');
...
function achFormatterTransform(ach) {
  ...
  const fileHeader = outputFields(ach.file, formats.fileHeader)
  formattedOutput.push(fileHeader);

  ach.batches.forEach((batch) => {
    const batchHeader = outputFields(batch, formats.batchHeader)
    formattedOutput.push(batchHeader)

    var format = formats[batch.entryClassCode];
    
    batch.entries.forEach((entryObj) => {
      let entry = outputFields(entryObj, format.entry)
      formattedOutput.push(entry)
      
      if (entry.addenda != null) {
        const addenda = outputFields(entryObj.addenda, format.addenda)
        formattedOutput.push(addenda)
      } else if (entryObj.addendas != null) {
        entryObj.addendas.forEach((addendaObj) => { 
          let addenda = outputFields(addendaObj, format.addenda)
          formattedOutput.push(addenda)
        });
      }
    });

    const batchFooter = outputFields(batch.footer, formats.batchFooter)
    formattedOutput.push(batchFooter)
  });

  const fileFooter = outputFields(ach.file.footer, formats.fileFooter)
  formattedOutput.push(fileFooter)


  formattedOutput = formattedOutput.filter(line => line !== "");

console.log(formattedOutput);

  let lineCount = formattedOutput.length
  var linesNeeded = 10 - (lineCount % 10);
  for (var i = 0; i < linesNeeded; i++) {
    formattedOutput.push(nines);
  }
